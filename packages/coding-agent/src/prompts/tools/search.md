...
- If the search is open-ended, requiring multiple rounds, break it into successive `search`/`find`/`read` passes (or delegate a bounded `executor` subagent) instead of chaining ad-hoc `search` calls blindly.
</critical>
